...
const isProfile = (value: unknown): value is Profile => {
  ...
  return (
    ...
    typeof profile.incognito === 'boolean'
  );
};

const parseStoredProfiles = (stored: string | null) => {
  if (!stored) {
    return defaultProfiles();
  }

  const parsed: unknown = JSON.parse(stored);
  return Array.isArray(parsed) ? parsed.filter(isProfile) : defaultProfiles();
};
...
export default function App() {
  ...
  useEffect(() => {
    AsyncStorage.getItem(STORAGE_KEY)
      .then((stored) => setProfiles(parseStoredProfiles(stored)))
      .catch(() => setProfiles(defaultProfiles()))
